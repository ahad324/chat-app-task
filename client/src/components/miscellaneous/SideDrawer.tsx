...
const SideDrawer = () => {
  ...
  const serverUrl: string = import.meta.env.VITE_API_URL || 'http://localhost:3000';
  ...
  const handleSearch = async () => {
    try {
      ...
      alert(`Error Occurred! Failed to load the search results Error:${error}`);
      setLoading(false);
    }
  };
  ...
  const accessChat = async (userId: string) => {
    try {
      ...
      alert(`Error fetching the chat Error:${error}`);
      setLoadingChat(false);
    }
  };
    ...
    <>
      <header className="flex items-center justify-between bg-white dark:bg-slate-800 p-2 border-b dark:border-slate-700">
        <button
          ...
          className="flex items-center space-x-2 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 px-3 py-1.5 rounded-md"
        >
